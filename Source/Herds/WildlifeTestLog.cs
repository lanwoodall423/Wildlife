...
namespace Herds
{
    ...
    {
        private const string Prefix = "[WildlifeTest][Herds]";
        ...
        public static TestRollMode HideOutcome { get; set; }
        ...
        private static void SetPartner(bool enabled)
        {
            try
            {
                Type type = AccessTools.TypeByName("Packs.WildlifeTestLog");
                if (type == null) return;
            ...
            {
                Log.Warning(Prefix + " Could not synchronize Packs and Predators diagnostic logging: " + exception.GetBaseException().Message);
            }
        }
