...
public static class PredatorBoldnessKillPatch
{
	public static void Prefix(Pawn __instance, DamageInfo? dinfo)
	{
		...
		if (__instance.RaceProps.Humanlike && __instance.Faction == Faction.OfPlayer && killer.RaceProps?.predator == true) component?.NotifyHumanConflict(killer, true);
		else if (__instance.RaceProps?.predator == true && killer.RaceProps.Humanlike && killer.Faction == Faction.OfPlayer) component?.NotifyHumanConflict(__instance, false);
	}
}
